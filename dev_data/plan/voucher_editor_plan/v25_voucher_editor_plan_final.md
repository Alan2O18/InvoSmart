# 憑證黏貼編輯器 — 究極計畫 v25 (合併展開 + 深層 UX/Edge-Case 釐清)

## 目標

將「憑證黏貼」從發票流程**完全解耦**，建立獨立編輯頁面。本計畫合併了 v21 所有的「極限防禦全展開」，並依照最新審核意見**無損追加**了詳細的 UX 情境表、前後端資料處理分工表，確保所有實作細節無任何歧義。
...
15. **金額極限防爆**：前端輸入與後端皆透過字串長度判斷（`len(str(amount)) > 7`）攔截大於 999 萬上限，拋出 ValueError。
...
27. **用途說明去重拼接**：只要拖入或移除發票（觸發計算），便提取當頁發票 `items[].category` 去重拼接為預設用途。(配合附錄 A 的 Override 防止覆蓋規則)。
...
34. **圖片跨專案白名單代理**：`/image/{jobId}` 除了還原 Job 實體路徑，更會驗證該 Job 屬於當前操作使用者的專案，否則 403 Forbidden。
35. **後端孤兒發票時序防護**：產 PDF 時增加 `if not os.path.exists(path): continue`，防最後一秒圖檔被刪引發 `FileNotFoundError`。
...
39. **底模 PNG RAM 快取 (@lru_cache)**：後端對底模 PDF 解析做快取，並將實體檔案修改時間 `os.path.getmtime()` 當作 Cache Key，確保拖曳畫布的 100 毫秒極速反應，且檔案遭替換時快取自動失效重繪。

### F. 效能與輸出品質 (Performance & IO) - 6 項
40. **前端縮圖代理 (Anti-OOM)**：`/image?thumb=true` 只回傳 800px 縮圖給 Canvas，防吃爆記憶體。
41. **上傳全局格式轉檔 (預留)**：上傳發票轉為 WebP/JXL 省空間。
42. **後端高畫質還原**：PDF Server 端直接調用高畫質原圖貼上。
43. **PDF 300DPI 尺寸壓縮**：插入圖片前，依據點陣佔比強制縮放為 300 DPI 像素量。計算公式：`target_px = (w_pts / 72) * 300`。縮放後再寫入文件，防止檔案過度膨脹被系統退件。
44. **無損 PDF 發布壓縮**：PyMuPDF 寫入時加上 `doc.save(deflate=True, garbage=4)` 做極致瘦身。
45. **二分搜尋 O(N log H) (自動排版)**：使用者點擊「自動排版」按鈕時觸發（非拖放時觸發）。O(N log H) 的 N=畫面上的發票數量，H=發票統一高度。`Max_width ≤ 535` 確保即使高度算好，超長明細的寬度也不會穿出安全區。
...
| 情境 | 系統行為 |
|:---|:---|
| **拖入/移除 發票更新欄位** | 發票放置於滑鼠放下點 (Drop Position)。清單上該項目立刻反灰 (Disabled)，左側面板重新計算該頁總金額、最晚日期與用途拼接。 |
| **用途說明的手動覆蓋保護** | 前端設有 `isManuallyEdited` 標記。若使用者手動修改過用途，再次拖入/移除發票時，跳出警告：「發現新發票，是否要用系統推薦字串覆蓋您手動修改的用途？」若告知不保留，則覆蓋並清除標記。 |
| **自動串號的即時聯動計算** | 只要 Prefix 或 Start Index 變更，或是發票的增減導致 `receiptCount` 張數變化，皆即時 (Reactive) 觸發重新計算**所有頁面**的 `voucherNo` (例如 `D-16-01~03`)，不需等待存檔。 |
| **切換憑證頁面 (Tab)** | 儲存當前頁 Canvas State (存入記憶體)，呼叫 `canvas.clear()`，重新載入目標頁 images 與文字至畫布，重算側邊欄。 |
| **偵測到非法/無效日期** | 當前頁面的「支付日期」欄位字體爆紅。右上角「產出 PDF」按鈕強制Disabled。<br>提示：*「偵測到無效日期。請將異常發票移出畫布，或退回發票審核頁面修正來源資料。」* |
| **偵測到小數點金額** | 當前頁面的「總金額/支付金額」欄位背景變黃 🟡。產出鎖死。<br>提示：*「依法規台幣不可有角分。請退回發票審核頁面修正來源資料為整數。」* |
| **發票在畫布上重疊** | 發票的 BBox 邊框變為紅色以示警告。但不鎖定產出鈕（允許使用者刻意推疊）。 |
| **背景草稿自動儲存 (Auto-Save)**| 編輯器開啟後，**每 30 秒** 或 **跨頁面切換時**，自動背景觸發 `POST /layout` 儲存草稿，防止瀏覽器意外關閉導致進度全失。 |

### B. 前後端資料處理分工表 (Data Processing Division)
| 欄位 / 功能 | 前端 (Vue) 送出欄位型態 | 後端 (FastAPI/PyMuPDF) 接收處理邏輯 |
|:---|:---|:---|
| **`amount` (總金額)** | **純數字字串** `"4607"` | 前端與系統皆可直接透過字串長度 `len(str(amount)) <= 7` 判斷不超過 999 萬上限。後端執行 `rjust(7, '※')` 變為 `"※※※4607"`，轉換 7 格座標逐字寫入。 |
...
| **座標 `(x,y,w,h)`**| **純數字浮點數** `145.25` | 直接作為 `fitz.Rect` 參數插入圖片，或作為 300DPI 計算依據 (`w_pts / 72`)。 |

### C. 全面錯誤處理清單 (Comprehensive Error Handling)
| 錯誤情境 | HTTP Status | 系統行為與提示訊息 |
|:---|:---|:---|
| **底模檔案損壞/不存在** | `500 Internal Server Error` | Backend 記錄 Error Log。前端提示：「系統底模遺失或損毀，請聯絡系統管理員修復環境。」 |
| **單一發票圖片損壞** | `200 OK` (不中斷產出) | 後端不會拋錯中斷，而是**在原本該發票的座標上，畫一個顯眼的「紅色大叉 (X)」，並印出提示文字「圖片損壞無法載入」**，確保輸出文件且有留存報錯證據。 |
| **金額超過七位數(百萬)上限** | `422 Unprocessable Entity` | PDF 拒絕生成。前端提示：「總金額不可超過限制，請拆分請款憑證。」 |
| **越權/非法操作專案發票** | `403 Forbidden` / `404` | 該使用者嘗試代理非自己專案的 `jobId` 圖片，立刻阻擋。 |

### D. 多頁管理操作細節 (Pagination Limits)
- **新增頁面**：點擊 Tab 列的 `[+] 新增憑證頁` 手動新增一頁空白底板。
- **頁數上限**：暫限制最多 `10` 頁。
- **空白頁儲存與產出**：空頁面可以正常儲存 Layout，**但後端產出時會自動略過完全沒有任何 Images 的頁面**，避免印出廢紙。
- **刪頁補號**：刪除頁面後，剩餘的 `pages[]` 陣列自動重排索引 (Index)，左側的憑證編號 即時聯動刷新重新串號。
