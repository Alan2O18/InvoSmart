# Job Repository - 資料存取層 (VLM-First 簡化版)
"""
...
class JobRepository:
    """
    Data access layer for job persistence (VLM-First).
    ...
    def insert_job(self, job_id: str, image_path: str, status: str = "ready") -> str:
        ...
        with self.lock:
            ...
            cur.execute(
                """
            INSERT OR REPLACE INTO jobs(job_id, image_path, status, updated_at)
        ...
        with self.lock:
            ...
            cur.execute(
                """UPDATE jobs SET 
                   ...
                   WHERE job_id=?""",
                (vlm_json, validation_json, stats_json, 1 if qr_verified else 0, job_id)
            )
            affected = cur.rowcount
            conn.commit()
            conn.close()
    ...
    def count_jobs(self) -> Dict[str, int]:
        ...
        with self.lock:
            conn = self._get_conn()
            cur = conn.cursor()
            cur.execute("SELECT status, COUNT(1) as cnt FROM jobs GROUP BY status")
            rows = cur.fetchall()
            conn.close()
        ...
        with self.lock:
            ...
            cur.execute(
                """SELECT job_id FROM jobs 
                   WHERE status IN ('ready', 'pending', 'running') LIMIT 1"""
            )
            row = cur.fetchone()
            conn.close()
