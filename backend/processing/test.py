...
class ReceiptSplitter:
    ...
    def split(
        ...
        for i, contour_info in enumerate(final_contours):
            ...
            window_name = (
                f"Receipt {i+1}/{len(final_contours)} | (r)Rotate (s)Save (q)Quit"
            )
            ...
            while True:
                key = cv2.waitKey(0) & 0xFF
                if key == ord("r"):  # 旋轉
                    warped_invoice = cv2.rotate(warped_invoice, cv2.ROTATE_90_CLOCKWISE)
                    preview_image = cv2.rotate(preview_image, cv2.ROTATE_90_CLOCKWISE)
                    cv2.imshow(window_name, preview_image)
                elif key == ord("s"):  # 儲存
                    final_receipt.append(warped_invoice)
                    print(f"     -> 已保留第 {i+1} 張發票")
                    break
                elif key == ord("q"):  # 跳過
                    print(f"     -> 已跳過第 {i+1} 張發票")
                    break
