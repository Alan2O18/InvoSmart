...
async def test_detect_raw_sub_rects_not_found(tmp_path):
    service, _, _ = _make_service(tmp_path)
    with patch("backend.engine.image_service.resolve_raw_source_by_filename", return_value=None):
...
async def test_detect_job_sub_rects_source_missing(tmp_path):
    ...
    with patch("backend.engine.image_service.resolve_resplit_raw_source", return_value=None):
        with pytest.raises(FileNotFoundError):
...
async def test_detect_job_sub_rects_read_fail(tmp_path):
    ...
    with patch("backend.engine.image_service.resolve_resplit_raw_source", return_value=src), patch(
        "backend.engine.image_service.utils.cv_imread_chinese", return_value=None
    ):
...
async def test_apply_raw_resplit_success(tmp_path):
    ...
    with patch("backend.engine.image_service.resolve_raw_source_by_filename", return_value=src), patch(
        "backend.engine.image_service.utils.cv_imread_chinese", return_value="img"
    ), patch("backend.engine.image_service.job_matches_raw_filename", return_value=True):
