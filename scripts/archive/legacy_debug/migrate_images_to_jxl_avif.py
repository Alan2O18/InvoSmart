...
def run(args) -> int:
    ...
    skipped = 0
    errors = 0
    # Track newly generated files so the caller can roll back if needed.
    new_files: list[str] = []
...
def _write_rollback(path: Path, new_files: list[str]):
    """Persist the list of newly created files so they can be bulk-deleted on rollback."""
...
def _write_report(
    ...
    *,
    ...
    """Write a structured JSON summary report."""
    payload = {
        "migrated": migrated,
        "skipped": skipped,
        "errors": errors,
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=0,
        metavar="N",
        help="Stop after processing N images (0 = unlimited, for staged rollout testing)",
    )
    parser.add_argument(
        "--report-path",
        default=None,
